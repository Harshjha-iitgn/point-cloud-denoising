...
class PairedPatchDatasetNPZ(Dataset):
    def __init__(self, root, split="train") -> None:
        ...
        if not os.path.exists(path):
            path = root  # allow direct folder
        self.files = [f for f in os.listdir(path) if f.endswith((".npy", ".npz"))]
        self.root = path
        self.split = split
    ...
    def __getitem__(self, idx):
        ...
        # handle both .npz and .npy structures
        if isinstance(data, np.lib.npyio.NpzFile):
            noisy = data["lr_points"]
            clean = data["hr_points"]
            center = data.get("center", np.zeros(3))
            scale = data.get("scale", np.ones(1))
        else:
            arr = np.asarray(data)
            if arr.ndim == 2 and arr.shape[1] >= 6:
                noisy, clean = arr[:, :3], arr[:, 3:6]
            else:
                noisy, clean = arr[:, :3], arr[:, :3]
            center = np.zeros(3)
            scale = np.ones(1)

        sample = {
            "noisy_points": torch.FloatTensor(noisy),
            "clean_points": torch.FloatTensor(clean),
...
# ---------------------------------------------------------------------
# Wrapper to make PairedPatchDatasetNPZ compatible with older training scripts
# ---------------------------------------------------------------------
class PUNetDatasetWrapper(PairedPatchDatasetNPZ):
    def __init__(self, root_dir=None, split="train", npoints=None, augment=False, **kwargs):
        root = root_dir if root_dir is not None else kwargs.get("root", None)
    ...
    def __getitem__(self, idx):
        ...
        if self.npoints is not None and sample["noisy_points"].shape[0] > self.npoints:
            sel = np.random.choice(sample["noisy_points"].shape[0], self.npoints, replace=False)
            for k in ["noisy_points", "clean_points"]:
                sample[k] = sample[k][sel]

        # optional augmentation
        if self.augment:
            sample["noisy_points"] = sample["noisy_points"] + 0.002 * torch.randn_like(sample["noisy_points"])
