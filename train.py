...
def train(cfg: DictConfig) -> None:
    ...
    # ---- load diffusion model (auto-handles AE integration) ----
    model, ckpt = load_diffusion(cfg)

    # ADDED: confirm AE checkpoint being used
    if is_main_process and hasattr(cfg.model, "ae_ckpt"):
        logger.info(f"Using pretrained AE checkpoint: {cfg.model.ae_ckpt}")
    ...
    for step in range(cfg.start_step, cfg.training.steps):
        ...
        for accum_iter in range(cfg.training.accumulation_steps):
            ...
            out = model(x_gt, x1=x_start, x_cond=x_cond)
