...
# ==============================================================
# FIXED SECTION: remove double normalization for P2F
# ==============================================================
def point_mesh_bidir_distance_single_unit_sphere(pcl, verts, faces):
    assert pcl.dim() == 2 and verts.dim() == 2 and faces.dim() == 2
    verts, center, scale = normalize_sphere(verts.unsqueeze(0))
    verts = verts[0]

    # ---- FIX: check if pcl already normalized (avoid double scaling)
    pcl_range = pcl.abs().max().item()
    if pcl_range > 1.5 or pcl_range < 0.5:
...
class Evaluator(object):
    def __init__(
        ...
        test_root = os.path.join(dataset_root, dataset, "pointclouds", "test")
        for res in ["50000_poisson", "30000_poisson", "10000_poisson", "poisson"]:
            cand = os.path.join(test_root, res)
            if os.path.isdir(cand):
                self.gts_pcl_dir = cand
                logger.info(f"[Evaluator] Using ground-truth pcl dir: {self.gts_pcl_dir}")
                break
        else:
            self.gts_pcl_dir = test_root
            logger.warning(f"[Evaluator] No Poisson folder found. Using {self.gts_pcl_dir}")
    ...
    def run(self):
        ...
        for name in tqdm(pcls_name, desc="Evaluate"):
            ...
            if name not in pcls_high:
                logger.warning("Shape `%s` not found, ignored." % name)
                continue
